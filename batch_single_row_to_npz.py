...
def parse_row_values(values):
    if len(values) != EXPECTED_T * 2 + 1:
        raise ValueError(f"Línea con longitud {len(values)}, esperaba {EXPECTED_T*2+1}.")
...
def main():
    ...
    with open(inp, "r") as f:
        for i, line in enumerate(f):
            ...
            if args.start: meta["start_date"] = args.start
            out_path = out_dir / f"meter_{i:06d}.npz"
            np.savez_compressed(out_path, X=X, meta=json.dumps(meta))
            count += 1
            if args.limit > 0 and count >= args.limit:
                break
            if args.progress > 0 and (i+1) % args.progress == 0:
                print(f"[INFO] Procesadas {i+1} filas...", file=sys.stderr)
    print(f"[OK] Hecho. Contadores procesados: {count}. Salida: {out_dir}")
